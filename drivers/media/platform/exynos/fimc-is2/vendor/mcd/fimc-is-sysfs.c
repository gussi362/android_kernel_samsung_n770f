...
#ifdef CAMERA_REAR2_TILT   /* TEMP_MAKALU_V2 */
		...
		struct device_attribute *attr, char *buf)
{
	...
	dev_info(dev, "%s: E : %d ", __func__, specific->sensor_id[SENSOR_POSITION_REAR]);
		...
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d\n", specific->sensor_id[SENSOR_POSITION_REAR2]);
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d\n", specific->sensor_id[SENSOR_POSITION_REAR3]);
}
...
static ssize_t camera_rear_paf_offset_mid_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
#ifdef FROM_PAF_OFFSET_MID_ADDR
	char *cal_buf;
	char tempbuf[10];
	int i;
	...
	for (i = 0; i < FROM_PAF_OFFSET_MID_SIZE / 8 - 1; i++) {
		sprintf(tempbuf, "%d,", *((s16 *)&cal_buf[FROM_PAF_OFFSET_MID_ADDR + 8 * i]));
		strncat(buf, tempbuf, strlen(tempbuf));
		memset(tempbuf, 0, sizeof(tempbuf));
	}
...
#ifdef ROM_PAF_CAL_DATA_START_ADDR
	memcpy(data, &cal_buf[ROM_PAF_CAL_DATA_START_ADDR + ROM_PAF_CAL_ERR_CHECK_OFFSET], 4);
#endif
	paf_err_data_result = *data | ( *(data + 1) << 8) | ( *(data + 2) << 16) | (*(data + 3) << 24);
